...
class searcher:
    ...
    def getscoredlist(self, results, words, a=1, b=1 ,c=1):
    ...
    def query(self,q,a=1,b=1,c=1):
        results, words = self.getmatchingpages(q)
        if len(results) == 0:
            print ('No matching pages found!')
            return

        scores = self.getscoredlist(results,words, a, b, c)
        return sorted([(score, url) for (url, score) in scores.items()], reverse=True)
